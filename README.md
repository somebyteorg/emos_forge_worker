...
EMOS forge 媒体处理 worker。它负责从本地路径或 HTTP/HTTPS 地址读取视频，生成视频、音频、字幕、雪碧图等产物，写入 `manifest.json` 和 `log.json`，并在 worker 模式下上传后台指定的资源。
...
如果不传 `--job-uuid` 或 `--root`，`upload` 会进入交互输入，并询问是否删除已上传文件，默认不删除。手动上传前会检查 `manifest.json`，如果包含未加密视频会拒绝上传。
